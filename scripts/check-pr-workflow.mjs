...
const workflow = fs.readFileSync(new URL('../.github/workflows/pr-commands.yml', import.meta.url), 'utf8');
const config = fs.readFileSync(new URL('../examples/coinpay.yml', import.meta.url), 'utf8');
...
test('reusable entrypoint requires the caller event and passes only named secrets', () => {
  assert.match(workflow, /on:\s*\n\s+workflow_call:/);
  assert.match(workflow, /COINPAY_API_KEY:\s*\n\s+required: false/);
